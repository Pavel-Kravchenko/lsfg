...
#######################################################################################################
Patients_common_table_2 <- Patients_common_table %>% merge(paired_pb_sf_cd8 %>% select(donor, B27, B38, C12), by="donor")
...
b <-ggplot(Patients_common_table_2 %>%
  filter(V%in% HUMAN_TRBV) %>% 
    filter(B27=="1") %>% 
  mutate(V_names=gsub("TRBV([0-9]?.+).*", "\\1", V)),aes(x=V_names, fill=count)) + 
...
b <- ggplot(B27_Patients_common_table_2 %>% group_by(V) %>% summarise(count = sum(count)) %>% mutate(V_names=gsub("TRBV([0-9]?.+).*", "\\1", V)), aes(x=V_names, y=count)) +
  geom_bar(stat="identity", colour="black", fill="#FFFFFF")  +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  guides(fill=FALSE)
...
b <- ggplot(B38_Patients_common_table_2 %>% group_by(V) %>% summarise(count = sum(count)) %>% mutate(V_names=gsub("TRBV([0-9]?.+).*", "\\1", V)), aes(x=V_names, y=count)) +
  geom_bar(stat="identity", colour="black", fill="#FFFFFF")  +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  guides(fill=FALSE)
...
b <- ggplot(All_Patients_common_table_2 %>% group_by(V) %>% summarise(count = sum(count)) %>% mutate(V_names=gsub("TRBV([0-9]?.+).*", "\\1", V)), aes(x=V_names, y=count)) +
  geom_bar(stat="identity", colour="black", fill="#FFFFFF")  +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  guides(fill=FALSE)
...
# b <-ggplot(Patients_common_table_2 %>%
#   filter(V%in% HUMAN_TRBV) %>% 
#     filter(B27=="1"),aes(x=V_names, fill=count)) + 
#   labs(title = "Density of distribution by V genes in B27+", 
#        x = "V genes", y = "density")+
#   geom_bar(alpha=.8, colour="black", fill="#FFFFFF") + 
#   theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
...
t_p <- Patients_common_table_2 %>%
  ...
  select(V, donor, pbinom), by=c("V", "donor")) %>% #group_by(donor) %>% summarise() %>%  View()
  group_by(V) %>% mutate(p_sum = round(mean(pbinom),4)) %>% 
  dcast(V~donor,value.var = "pbinom") %>% 
  mutate(N_enriched=rowSums(.[2:ncol(.)]<0.05)   ) %>%
